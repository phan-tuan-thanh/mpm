        ...
        @if (projectStore.isLoading() && isOnProjectDetail() && !projectStore.currentProject()) {
          ...
          <div class="flex h-full w-full items-center justify-center bg-white dark:bg-surface-900 flex-1">
            <div class="text-center">
              ...
              <p class="mt-4 text-sm text-gray-500 dark:text-surface-400 font-medium">{{ t().loadingProject }}</p>
            </div>
          </div>
          ...
          <div class="flex h-full w-full flex-col items-center justify-center bg-white dark:bg-surface-900 p-6 text-center flex-1">
            ...
            <h2 class="mt-6 text-2xl font-bold tracking-tight text-gray-900 dark:text-surface-0">{{ t().noAccess }}</h2>
            <p class="mt-2 text-base text-gray-600 dark:text-surface-400">{{ t().noAccessDetail }}</p>
            <div class="mt-8">
              <button
                (click)="goBack()"
...
export class AppShellComponent implements OnInit {
  ...
  readonly t = computed(() => {
    ...
    return isEn ? {
      ...
      noAccessDetail: 'You are not a member of this project or the project does not exist.',
      backToProjects: 'Back to project list'
    } : {
      loadingProject: 'Đang tải dự án...',
      noAccess: 'Không có quyền truy cập',
      noAccessDetail: 'Bạn không phải là thành viên của dự án này hoặc dự án không tồn tại.',
