...
@Component({
  ...
  template: `
    <div class="flex h-screen w-screen overflow-hidden bg-[#fafbfe] font-sans antialiased text-[#2d3748]">
      @if (projectStore.isLoading() && isOnProjectDetail() && !projectStore.currentProject()) {
        ...
        <div class="flex h-full w-full items-center justify-center bg-white">
          <div class="text-center">
            ...
            <p class="mt-4 text-sm text-gray-500 font-medium">Đang tải dự án...</p>
          </div>
        </div>
      } @else if (projectStore.error()) {
        <!-- Error Page: Forbidden or Not Found -->
        <div class="flex h-full w-full flex-col items-center justify-center bg-white p-6 text-center">
          ...
          <h2 class="mt-6 text-2xl font-bold tracking-tight text-gray-900">Không có quyền truy cập</h2>
          <p class="mt-2 text-base text-gray-600">Bạn không phải là thành viên của dự án này hoặc dự án không tồn tại.</p>
          <div class="mt-8">
            <button
              (click)="goBack()"
              class="rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500 transition"
            >
