...
export class TaskDetailPanelComponent implements OnInit, OnDestroy {
  readonly taskStore = inject(TaskStore); readonly projectStore = inject(ProjectStore);
  private readonly attachmentService = inject(AttachmentService); private readonly taskService = inject(TaskService);
  private readonly linkService = inject(LinkService); private readonly relationService = inject(RelationService);
  private readonly route = inject(ActivatedRoute); private readonly router = inject(Router);
  private readonly messageService = inject(MessageService); private readonly destroy$ = new Subject<void>();
  readonly task = this.taskStore.currentTask; readonly isVisible = signal(false);
  protected editTitle = ''; protected activity = signal<TaskActivity[]>([]); protected currentUserId = signal('');
  ...
  ngOnInit(): void {
    this.route.queryParams.pipe(takeUntil(this.destroy$)).subscribe((params) => {
      ...
      if (taskId && projectId) {
        this.isVisible.set(true); this.taskStore.loadTask(projectId, taskId);
