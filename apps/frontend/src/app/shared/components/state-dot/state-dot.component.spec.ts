...
describe('StateDotComponent', () => {
  ...
  beforeEach(() => {
    ...
    component = TestBed.runInInjectionContext(() => new StateDotComponent());
  });

  describe('isFilled — dot tô đặc theo group', () => {
    it.each(['started', 'completed'])('group %s → filled', (group) => {
      component.state = { name: 'X', colorLight: '#10B981', colorDark: '#10B981', group };
  ...
  describe('iconColor — pi icon tô màu state, emoji giữ màu gốc', () => {
    it('pi icon → màu state', () => {
      component.state = { name: 'Done', colorLight: '#10B981', colorDark: '#10B981', group: 'completed', icon: 'pi pi-check-circle' };
      expect(component.iconColor).toBe('#10B981');
    });

    it('icon dạng "pi-..." (thiếu prefix "pi ") vẫn là pi icon', () => {
      component.state = { name: 'Done', colorLight: '#10B981', colorDark: '#10B981', group: 'completed', icon: 'pi-check-circle' };
      expect(component.iconColor).toBe('#10B981');
    });

    it('emoji → null (không override màu)', () => {
      component.state = { name: 'Done', colorLight: '#10B981', colorDark: '#10B981', group: 'completed', icon: '✅' };
      expect(component.iconColor).toBeNull();
    });

    it('không có icon → null', () => {
      component.state = { name: 'Todo', colorLight: '#9CA3AF', colorDark: '#9CA3AF', group: 'unstarted' };
      expect(component.iconColor).toBeNull();
    });
  });
