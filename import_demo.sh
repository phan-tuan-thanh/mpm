...
# ─── 1. Generate JWT Token for admin@authentik.local ──────────────────────
echo -e "${BLUE}[INFO]${NC} Đang khởi tạo phiên làm việc cho quản trị viên..."
PRIVATE_KEY_PATH="keys/private.pem"
if [ ! -f "$PRIVATE_KEY_PATH" ]; then
  echo -e "${RED}[ERROR]${NC} Không tìm thấy RSA private key tại: $PRIVATE_KEY_PATH"
  echo -e "Hãy đảm bảo dự án đã được chạy và khởi tạo qua ./start.sh."
  exit 1
fi

TOKEN=$(cd apps/backend && node -e "
const fs = require('fs');
...
try {
  ...
  const payload = {
    sub: 'd948734d-39e6-48ad-8344-696f9c20d18b',
...
if [ -z "$TOKEN" ]; then
  echo -e "${RED}[ERROR]${NC} Không thể tạo JWT Token."
  exit 1
fi
echo -e "${GREEN}[OK]${NC} JWT Token khởi tạo thành công."
...
# The creator admin@authentik.local is added automatically as Scrum_Master during project creation.
