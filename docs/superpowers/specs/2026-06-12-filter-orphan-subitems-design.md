...
Hai nguyên nhân chồng nhau (backend lọc đúng — xác minh SQL trực tiếp: 55 task có `sprint_id` = Sprint 1):

1. **Tầng request:** `backlog.component.ts:296` `reloadBacklog()` ở List view luôn ép `{ parentId: null }`, ghi đè logic `hasActiveFilters` của `task.store.loadBacklog` (store vốn bỏ ràng buộc parentId khi có filter, nhưng `if ('parentId' in query)` cho phép caller override). API bị hỏi "task gốc thuộc sprint" → rỗng. Board truyền `parentId: undefined` nên không bị.
2. **Tầng render:** `task-list.component.ts:237`: `rootTasks = this._tasks().filter((t) => !t.parentId)` — chỉ task không có parent được render làm root; task match filter nhưng parent vắng mặt trong kết quả ("mồ côi") không bao giờ được render. Board view không bị (dùng toàn bộ tasks, không lọc root).
...
### 3. Bỏ ép `parentId` ở List view

`backlog.component.ts` `reloadBacklog()`: nhánh list gọi `loadBacklog(projectId)` không truyền `parentId` — store tự quyết theo `hasActiveFilters` (có filter → mọi cấp, không filter → chỉ root). Nhánh board giữ nguyên `{ parentId: undefined }`.

### 4. Không đụng backend, không đụng Board view
